...
echo "%avrdude%" -C"%conf%" -p%device% -c%prog% -b%baud% -D %verbose% -Uflash:w:"%hexfile%":i -P%port% 2>&1

"%avrdude%" -C"%conf%" -p%device% -c%prog% -b%baud% -D %verbose% -Uflash:w:"%hexfile%":i -P%port% 2>&1
